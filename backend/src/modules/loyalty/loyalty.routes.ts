...
router.post('/add', authenticate, authorize(UserRole.ADMIN), validate(z.object({ user_id: z.number().int(), points: z.number().int(), source: z.string() })), async (req, res, next) => {
  try { const { query: q } = require('../../config/database'); await q('UPDATE Points SET balance=balance+@p, lifetime_earned=lifetime_earned+@p, updated_at=GETDATE() WHERE user_id=@uid', { p: req.body.points, uid: req.body.user_id }); res.json({ success: true, message: 'Points added' }); } catch (e) { next(e); }
});
